    ...
    public class IKSolver5DOF : Kinematic
    {
        ...
        protected override double[] getJointAngles(Point3D Position)
        {
            ...
            do
            {
                ...
                Ec = deltaO + Math.Pow(Vector3D.DotProduct(Vector3D.Subtract(Pd, Ph), Vector3D.Subtract(Pd, Ph)), 2);
        ...
        protected override double[] getJointAngles(Point3D Position, Point3D Orientation, double[,] RotM)
        {
            ...
            do
            {
                ...
                if (Ec > eps) // begin Cyclic Coordinate Descent loop
                {
                    ...
                    k3 = Vector3D.DotProduct(frame[link, 2], Vector3D.Add(wp * Vector3D.CrossProduct(Pih[link], Pid), ko3));
                    //k3 = Vector3D.DotProduct(frame[link, 2], Vector3D.Add(wp * Vector3D.CrossProduct(Pid, Pih[link]), ko3));
                    double phi;
                    // minimize position and orientation error
                    if ((k1 - k2) != 0)
                    ...
                    double phiMax = MinMax[link - 1].Y - radAngle[link];
                    double phiMin = MinMax[link - 1].X - radAngle[link];
        ...
        public void function1_grad(double[] q, ref double func, double[] grad, object obj)
        {
            ...
            func = deltaO + Math.Pow(Vector3D.DotProduct(Vector3D.Subtract(Pd, Ph), Vector3D.Subtract(Pd, Ph)), 2);
